...
//       firstName = wuTangMembers[Math.round(Math.random() * (wuTangMembers.length - 5))]
//       lastName = wuTangMembers[Math.round(Math.random() * (wuTangMembers.length - 5))]
...
export function generateUsernames(numUsers: number) {
  const userNames: string[] = []
  while (userNames.length < numUsers + 1) {
    ...
    const exists = userNames.filter((name) => name == username)
    if (exists.length < 1) userNames.push(username)
  }
  return userNames
}
