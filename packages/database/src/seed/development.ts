...
import loadGames from './loadgames' // Assuming .js extension or your TS setup handles .ts
import loadProducts from './seedProducts' // Assuming .js extension
...
async function seedGamesInternal(): Promise<void> {
  ...
  try {
    const gamesFromScript = await loadGames(prisma, createdOperators[0]) // Pass createdGameProviders
...
async function seedProductsInternal(): Promise<void> {
  ...
  try {
    const tempProducts = await loadProducts(prisma, createdOperators[0], createdCurrencies[0].id)
    if (Array.isArray(tempProducts)) {
      // Fetch the products from the database to get the correct type
