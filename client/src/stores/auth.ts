...
export const useAuthStore = defineStore(
  ...
  () => {
    ...
    function setAuthData(data: AuthResponseDto) {
      ...
      refreshToken.value = data.refreshToken as string // Ensure refreshToken is always a string or null
      currentUser.value = data.user // Create a copy to avoid unintended modifications
      // Use a more robust way to handle default avatar
      // if (currentUser.value && currentUser.value.image === undefined) {
      //   currentUser.value.image = 'avatar-10.webp';
      // }
