...
const communication = eventSystem.create();
...
const ORDERED_EVENTS = ['currency', 'betLevels', 'operatorFreeBetMessages', 'freeBets', 'balance', 'balances'];
...
function makeData(options) {
    ...
    if(options.jurisdiction) {
        data.jurisdiction = typeof options.jurisdiction === 'object' ? JSON.stringify(options.jurisdiction) : options.jurisdiction;
    }
...
communication.connect = function(options) {
    ...
    request.onload = () => {
        ...
        if(response.error) {
            ...
            this.websocket.on('message', message => this.trigger('debug', {'communication.game': message}));
