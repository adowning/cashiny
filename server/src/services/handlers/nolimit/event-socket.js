    ...
    if (ws !== null && ws.readyState === WebSocket.OPEN) {
      ...
      setTimeout(() => {
        if (dataQueue.length > 0) {
          client.trigger(
            'debug',
            'Timeout after reconnecting at ' + TIMEOUT + 'ms, clearing and trying again'
          );
          clearSocket();
          createSocket();
        }
      ...
      // Connecting
      setTimeout(() => {
        if (ws.readyState === WebSocket.CONNECTING && dataQueue.indexOf(data) !== -1) {
          // Still connecting and data not sent
          client.trigger(
            'debug',
            'Timeout while connecting at ' + TIMEOUT + 'ms, clearing and trying again'
          );
          clearSocket();
          createSocket();
        }
