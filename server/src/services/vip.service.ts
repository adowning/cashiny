...
export async function getVipSignInInfo(userId: string): Promise<VipSignInStatus> {
  ...
  const nextRewardIndex = todayClaimed
    ? currentStreak % DAILY_SIGN_IN_REWARDS.length
    : (currentStreak - 1) % DAILY_SIGN_IN_REWARDS.length;
  const nextReward = DAILY_SIGN_IN_REWARDS[nextRewardIndex];
...
export async function claimDailySignIn(
  ...
  const vipInfo = await fetchUserVipInfo(userId); // Ensure VIP record exists
  ...
  const claimedReward = await prisma.$transaction(async (tx) => {
    const newRewardRecord = await tx.userReward.create({
      data: {
        userId,
        rewardType: RewardType.DAILY_SIGN_IN,
        description: rewardConfig.description || `Daily Sign-In: Day ${streakForToday}`,
        status: RewardStatus.CLAIMED,
        claimedAt: new Date(),
        amount: rewardConfig.amount,
        currencyId: rewardConfig.currencyId,
        metaData: { streak: streakForToday, day: rewardConfig.day },
        vipLevelRequirement: vipInfo.level,
      },
    });

    if (rewardConfig.xp) {
      await addXpToUser(userId, rewardConfig.xp, 'DAILY_SIGN_IN_REWARD');
    }
      ...
      await createTransactionRecord(
        {
          ...
          currencyId: rewardConfig.currencyId,
          description: `Daily Sign-In Reward: ${rewardConfig.description}`,
          // balanceBefore, balanceAfter would be handled by createTransaction if it updates wallet
        },
        tx
      );
    }
...
export async function claimCycleReward(
  ...
  let vipInfoUpdateData: Partial<PrismaVipInfo> = {}; // To update VipInfo last claim timestamp
...
export async function fetchVipTasks(userId: string): Promise<VipTaskInfo[]> {
  ...
  return tasks.map((task) => ({
    ...
    isCompleted: task.userTaskProgress[0]?.completed || false,
    isClaimed: !!task.userTaskProgress[0]?.claimedAt,
  }));
}
...
export async function fetchVipBetAwardList(userId: string): Promise<any[]> {
  console.warn(
